...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
    ...
    \STATE \COMMENT{Step 1: 获取控件树}
    \TRY
      \STATE \texttt{state.hierarchy\_xml $\leftarrow$ device.dump\_hierarchy()}
    \CATCH
      \STATE \texttt{state.hierarchy\_xml $\leftarrow$ "<error>获取控件树失败</error>"}
    \ENDTRY

    \STATE \COMMENT{Step 2: 获取当前 App 信息}
    \TRY
      \STATE \texttt{info $\leftarrow$ device.info}
      \STATE \texttt{state.current\_package $\leftarrow$ info.currentPackageName}
      \STATE \texttt{state.current\_activity $\leftarrow$ info.currentActivity}
    \CATCH
      \STATE \COMMENT{获取失败则字段保持为 None，不影响主流程}
    \ENDTRY

    \STATE \COMMENT{Step 3: 截图（可选）}
    \IF{\texttt{screenshot = True}}
      \TRY
        \STATE \texttt{counter $\leftarrow$ counter + 1}
        \STATE \texttt{filename $\leftarrow$ format("screen\_\{counter:04d\}\_\{timestamp\}.png")}
        \STATE \texttt{filepath $\leftarrow$ join(screenshot\_dir, filename)}
        \STATE \texttt{device.screenshot(filepath)}
        \STATE \texttt{state.screenshot\_path $\leftarrow$ filepath}
      \CATCH
        \STATE \PRINT "截图失败"
      \ENDTRY
    \ENDIF
  ...
  \begin{algorithmic}[1]
    ...
    \WHILE{\texttt{now() < deadline}}
      ...
      \IF{\texttt{package\_changed $\vee$ activity\_changed $\vee$ xml\_changed}}
        \STATE \COMMENT{变化了，重新带截图采集一次后返回}
...
def truncate_xml(xml, max_length=15000):
    ...
    truncated = xml[:max_length]      # 先简单截断
    
    last_node_end = truncated.rfind("</node>")
    if last_node_end > max_length * 0.8:
        truncated = truncated[:last_node_end + 7]   # 在完整节点处切断
...
\texttt{ScreenState} 是感知层的核心数据结构，使用 Python dataclass 定义：

\begin{lstlisting}[language=Python, caption={ScreenState 数据类定义}]
@dataclass
class ScreenState:
    """一次屏幕采样的完整状态。"""
...
\begin{lstlisting}[language=XML, caption={网易云音乐搜索页控件树片段（节选）}]
  ...
  <node index="0" text="" resource-id="" 
    ...
    <node index="0" text="" resource-id="" 
      ...
      <node index="1" text="" ...>
        ...
        <!-- 推荐标签栏 -->
        <node index="1" text="推荐" 
              class="android.widget.TextView" 
              clickable="true" 
              bounds="[50,160][120,190]"/>
        <node index="2" text="热歌榜" 
              class="android.widget.TextView" 
              clickable="true" 
              bounds="[150,160][250,190]"/>
        <node index="3" text="新歌榜" 
              class="android.widget.TextView" 
              clickable="true" 
...
以下是控件匹配（Widget Match）的 System Prompt 模板，展示了如何将感知层的控件树信息与 LLM 结合。

\begin{lstlisting}[language=Python, caption={控件匹配 System Prompt 模板（节选）}]
WIDGET_MATCH_SYSTEM = """你是一个安卓UI自动化专家。你需要根据用户的操作意图
和当前安卓屏幕的控件树（XML格式），找出应该操作的控件并确定操作类型。

控件树中每个节点包含以下属性：
- text: 控件显示的文本内容
- resource-id: 控件的资源ID（最稳定的选择器）
- class: 控件类型（android.widget.TextView等）
- content-desc: 无障碍描述
...
- 用户说「点击/选择/进入/播放/确认」→ 找对应按钮或条目 → click
- 用户说「点击键盘上的搜索/回车/完成」→ key_event（key_code=66）
  ⚠️ 重要：软键盘是独立窗口，控件树中看不到键盘按钮！
...

严格以如下 JSON 格式输出：
{{
  "action": "click|set_text|...|key_event",
  "target": {{
    "text": "...",
    "resource_id": "...",
    "class": "...",
